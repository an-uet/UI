...
function htmlItem($trealet_id,$nij, $css_input_id='',$idata, $itime, $timeline_item='', $timeline__content='', $timeline__img='', $timeline__content_title='', $timeline__content_desc='',$index_more){
	if(isset($idata['url_full']))//Show up the data item
	{
		...
		//Text
		if($ext=='TXT'){
			$vobj .= file_get_contents('https://trealet.com'.$url_full); //For embedded video
			//$vobj = '<iframe src="'.$url_full.'"></iframe>';
		}
		
		//Youtube
		if($ext=="YTB"){
			$vid = file_get_contents('https://trealet.com'.$url_full);
		...
		if($ext=='GLB'){
			$vobj .= '<div style="position: relative; width: 90%; height: 0; padding-bottom: 50%;">';
			$vobj .= '<iframe src="https://trealet.com/h3r/embed/?glb='.$url_full.'" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;"></iframe>';
			$vobj .= '</div>';
